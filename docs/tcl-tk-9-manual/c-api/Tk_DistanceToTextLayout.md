...
- **`int numChars`** *(in)*
The number of characters to consider from *string*.  If *numChars* is negative, then assumes *string* is null terminated and uses **Tcl_NumUtfChars** to determine the length of *string*.
...
A token that represents the cached layout information about the single-font, multi-line, justified piece of text.  This token is returned by **Tk_ComputeTextLayout**.
...
- **`int underline`** *(in)*
Index of the single character to underline in the text layout, or a negative number counting backwards from the end of the string. Any out-of-range number (e.g. INT_MIN) means no underline.
...
Postscript code that will print the text layout is appended to the result of interpreter *interp*.
