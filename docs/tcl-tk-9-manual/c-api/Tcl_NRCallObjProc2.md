...
Tcl_Command
Tcl_NRCreateCommand2(interp, cmdName, proc2, nreProc2, clientData,
                    deleteProc)
...
int
Tcl_NRCallObjProc2(interp, nreProc2, clientData, objc, objv)
...
Called in order to evaluate a command.  Is often just a small wrapper that uses **Tcl_NRCallObjProc** to call *nreProc* using a new trampoline.  Behaves in the same way as the *proc* argument to **Tcl_CreateObjCommand**(3) (*q.v.*).

- **`Tcl_ObjCmdProc2 *proc2`** *(in)*
Called in order to evaluate a command.  Is often just a small wrapper that uses **Tcl_NRCallObjProc2** to call *nreProc2* using a new trampoline.  Behaves in the same way as the *proc2* argument to **Tcl_CreateObjCommand2**(3) (*q.v.*).

- **`Tcl_ObjCmdProc *nreProc`** *(in)*
Called instead of *proc* when a trampoline is already in use.

- **`Tcl_ObjCmdProc2 *nreProc2`** *(in)*
...
Pointer to an unshared Tcl_Obj where the result of the evaluation is stored if the return code is TCL_OK.
