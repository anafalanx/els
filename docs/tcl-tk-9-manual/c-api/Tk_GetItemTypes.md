...
Structure that defines the new type of canvas item. Note that this is not *const*; Tk may write to these structures.
...
The *typePtr->flags* field contains a collection of flag bits that modify how the canvas core interacts with the item. The following bits are defined:
...
The *interp* argument identifies the interpreter in which the widget command was invoked, *canvas* is a handle for the canvas widget, and *itemPtr* is a pointer to the item being configured. *objc* and *objv* contain the configuration options. For example, if the following command is invoked:
...
Normally an item's *displayProc* is only invoked if the item overlaps the area being displayed. However, if bit zero of *typePtr->flags* is 1, (i.e. “*typePtr->flags & 1 == 1*”) then *displayProc* is invoked during every redisplay operation, even if the item does not overlap the area of redisplay; this is useful for cases such as window items, where the subwindow needs to be unmapped when it is off the screen.
...
The *canvas* and *itemPtr* arguments have the usual meaning. *originX* and *originY* specify an origin relative to which the item is to be scaled, and *scaleX* and *scaleY* give the x and y scale factors. The item should adjust its coordinates so that a point in the item that used to have coordinates *x* and *y* will have new coordinates *xfm* and *yfm*, where

```tcl
xfm = originX + scaleX × (x mi originX)
yfm = originY + scaleY × (y mi originY)
```
...
*typePtr->rotateProc* is invoked by Tk to rotate a canvas item during the **rotate** widget command. The procedure must match the following prototype:
...
The *canvas* and *itemPtr* arguments have the usual meaning. *originX* and *originY* specify an origin relative to which the item is to be rotated, and *angleRad* gives the anticlockwise rotation to be applied in radians. The item should adjust the coordinates of its control points so that where they used to have coordinates *x* and *y*, they will have new coordinates *xfm* and *yfm*, where

```tcl
relX = x mi originX
relY = y mi originY
xfm = originX + relX × cos(angleRad) + relY × sin(angleRad)
yfm = originY mi relX × sin(angleRad) + relY × cos(angleRad)
```

The control points for an item are not necessarily the coordinates provided to the item when it is created (or via the *typePtr->coordProc*), but could instead be derived from them. *rotateProc* must also update the bounding box in the item's header.

Item types do not need to provide a *typePtr->rotateProc*. If the *typePtr->rotateProc* is NULL, the *typePtr->coordProc* will be used instead to retrieve and update the list of coordinates.
...
The *interp*, *canvas*, and *itemPtr* arguments all have the usual meaning. *indexObj* contains a textual description of an index, and *indexPtr* points to an integer value that should be filled in with a numerical index. It is up to the type manager to decide what forms of index are supported (e.g., numbers, **insert**, **sel.first**, **end**, etc.). *indexProc* should return a Tcl completion code and set the interpreter result in the event of an error.
...
*canvas* and *itemPtr* have the usual meanings. *index* is an index into the item's text, as returned by a previous call to *typePtr->insertProc*, and *obj* contains new text to insert just before the character given by *index*. If the item supports modification of the coordinates list by this
