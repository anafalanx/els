...
- **`bool | int *boolPtr`** *(out)*
Points to place where **Tcl_GetBooleanFromObj** stores the boolean value (0 or 1) obtained from *objPtr*.

- **`char *charPtr`** *(out)*
Points to place where **Tcl_GetBoolFromObj** stores the boolean value (0 or 1) obtained from *objPtr*.

- **`int flags`** *(in)*
0 or TCL_NULL_OK. If TCL_NULL_OK is used, then the empty string or NULL will result in **Tcl_GetBoolFromObj** return TCL_OK, the *charPtr filled with the value **'\xFF'**;
