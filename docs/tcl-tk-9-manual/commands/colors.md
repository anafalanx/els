...
On macOS, the following additional system colors are available.  This first group contains all of the HIBrush colors available in the HIToolbox library. Note that on macOS 10.14 (Mojave) and later these colors are unlikely to match the color actually used for the purpose suggested by the color name.
...
The numbered systemWindowBackgroundColors below are used in the **ttk::notebook** and **ttk::labelframe** widgets to provide a contrasting background.  Each numbered color contrasts with its predecessor.
