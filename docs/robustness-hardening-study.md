...
- **Status — IMPLEMENTED** (the `CRASH RECOVERY / AUTOSAVE (R2)` section of `els.tcl` + the `els::win_lock_file`/`win_try_lock` helpers in `src/winfs.c`; 50+ in-process tests in `tests/recover.test` + a process-level recovery probe in `tools/probe_exe.tcl`). An adversarial design pass (then an adversarial code review) refined the sketch above on several points worth recording:
