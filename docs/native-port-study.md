...
>   project"). els now builds against z's **shared runtime under `<z>/.z/r`**
>   (`z tasks env` prints the roots — currently `C:\dev\.z\r`): Tcl/Tk 9 + the static
...
>   `C:/Users/anafa/dev/els/…`, read the corresponding `<z>/.z/r/…` payload.
> - **The legacy wish-wrapper build was removed** (commit c02345c): there is no
>   `z build-wish` task and `tools/exeicon.tcl` is deleted. `z build` is the native
>   development builder; the PE resources are generated by `tools/genres.tcl` +
>   `tools/mkico.tcl`.
> - **Runtime and release policy moved on too.** The historical first-run chooser
>   no longer exists: state is always beside `els.exe` (beside `els.tcl` for a
>   source run), with no profile fallback, migration, or deletion. Old profile
>   state is ignored. The only legacy bridge is a one-time copy from an adjacent
...
>   task runner rejects development output outside `build/` (including
>   `dist/els.exe`). The maintained verification sequence is `z build`,
>   `z probe-exe build/els-dev.exe`, then the release-only `z release-check` and
>   `z sign` gates.
>
> Also stale below but harmless: the `version=0.30` selftest string and the
> "268-test suite" count (the maintained suite now defines more than 700 tests). The architecture, link recipe, init order,
> and DPI/manifest decisions are all still accurate.

## 1. Executive Summary & Verdict

**The blunt truth first (historical baseline): Tcl and Tk were already statically linked into the shipped exe.** At the time of this study, `dist/els.exe` (5,744,031 B) was a byte-copy of the static Tk shell `.toolchain/tcl9s/bin/wish90s.exe` (5,591,049 B) with a 152,982 B zipfs payload appended. That retired `.toolchain` path is now `<z>/.z/r/tcltk/9.0.3/tcl9s`; the current product uses the custom `src/els_main.c` entry point and statically compiles its product modules instead of embedding `icudet.dll`.
...
**Historical pre-port baseline (obsolete; not the current `z build`):**
...
> **Historical plan, retained as engineering evidence.** These phases were
> completed and must not be followed literally now. In particular, the wrapper
> rollback commands and project-local `.toolchain` paths below no longer exist;
> use the current development and release flow in the status update above.
...
- **`z probe-exe build/els-dev.exe`** — the current explicit development-candidate form; it exercises first-launch adjacent-config resolution (and the absence of a chooser), session restore, explicit-arg open, recovery, and single-instance handoff via isolated standalone copies and a file-report channel.
...
Probe under `tclsh90`, never under `wish90`; GUI startup errors "rain" modal dialogs you can't read, so the file-report selftest is mandatory. The following sentence was the historical transition instruction and is now complete/obsolete: flip `task_build` to native, retain a temporary wrapper rollback, and promote the static-Tcl checks. Today there is no wrapper task; `z build` plus `z probe-exe build/els-dev.exe` is the development check, while publication goes through `z release-check` and `z sign`.
...
- **License.** els is MIT (`LICENSE`); Tcl/Tk is BSD-style. Static linking carries no copyleft obligation. The current packager embeds explicit top-level `LICENSE.txt` and `THIRD-PARTY-NOTICES.txt` files with the exact applicable Tcl, Tk, MinGW-w64 runtime, GCC GPLv3 plus runtime-exception, bundled zlib, and LibTomMath terms; the release gate byte-verifies them against their source files.
...
- **Promoted to the default build.** What the plan calls `task_build-native` shipped as the canonical **`z build`**. The wish-wrapper briefly survived as `z build-wish` for one rollback cycle and was then removed. The historical `z build dist/els.exe` form once worked, but the current runner explicitly rejects every output outside `build/`; only `z release-check` and `z sign` may publish under `dist/`.
...
- `tools/tasks.tcl` — current native development build, isolated process probes, PE policy checks, release reproducibility/signing gates, and journaled promotion; there is no `task_build-wish`.
- `tools/package.tcl` — `--wrapper` plus `zipfs lmkimg`; the current build appends the payload to the native bare executable only. The wish-wrapper branch is gone.
- `tools/exeicon.tcl` and `z build-wish` — removed historical icon-stamping/build paths, retained in this study only as migration history.
- `src/icudet.c` — compiled to `build/icudet.o` without `-DUSE_TCL_STUBS`.
- Historical `.toolchain/tclsrc/tk9.0.3/win/{winMain.c,wish.exe.manifest}` — templates used for `src/els_main.c` and the generated manifest; the project-local tree no longer exists.
- The former `.toolchain/tcl9s`, `.toolchain/tcl9`, and `.toolchain/msys64` inputs now resolve from z's shared `<z>/.z/r/...` payloads. Use `z tasks env` as the authoritative current path report rather than copying paths from this study.
