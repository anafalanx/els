...
get *calm* right — EditPad Pro, Sublime Text, Zed, Nova, iA Writer — adapted to
what Tk 9 can actually render well.

## Principles

1. **The text plane is sacred.** Nothing covers the page. Find/replace is a
   docked strip that *pushes* the text, never a modal dialog over it — the
   choice Sublime and EditPad Pro independently converged on. This is the *pro*
...
- **Leading ≈ 1.34×** — the single biggest "calm" lever. Applied via the Text
...
flat buttons; find toggles as grey "on" chips (neutral — red stays the caret's);
